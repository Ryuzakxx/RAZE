...
# ── CPU / RAM senza psutil ────────────────────────────────────────────────────

class _SysPoller(QThread):
    """Legge CPU e RAM tramite stdlib (subprocess / /proc), senza psutil."""
    ready = pyqtSignal(str, str)

    def run(self):
        cpu = self._cpu()
        ram = self._ram()
        self.ready.emit(cpu, ram)
    ...
    def _cpu(self) -> str:
        try:
            if sys.platform == "win32":
                ...
                # es: "CPU usage: 5.12% user, 3.45% sys, 91.43% idle"
                import re
                idle = re.search(r"([\d.]+)% idle", out)
                if idle:
                    return f"{100 - float(idle.group(1)):.0f}%"
                return "n/a"
            else:
                # Linux: legge /proc/stat due volte a distanza di 200ms
                import time
                s1 = self._proc_stat()
                time.sleep(0.2)
                s2 = self._proc_stat()
    ...
    def _ram(self) -> str:
        try:
            if sys.platform == "win32":
                ...
                # header: FreePhysicalMemory  TotalVisibleMemorySize
                # values: 4194304             8388608
                vals = lines[1].split() if len(lines) > 1 else []
                if len(vals) == 2:
                    free_kb  = int(vals[0])
                    total_kb = int(vals[1])
                    used_gb  = (total_kb - free_kb) / (1024 ** 2)
                    return f"{used_gb:.1f} GB"
                return "n/a"
            elif sys.platform == "darwin":
                import re
                # vm_stat per pages usate
                vm  = subprocess.check_output(
                    ["vm_stat"], timeout=3, stderr=subprocess.DEVNULL
                ).decode()
                page_size = 4096
                def _pages(pattern):
                    m = re.search(pattern, vm)
                    return int(m.group(1)) if m else 0
                active   = _pages(r"Pages active:\s+(\d+)")
                wired    = _pages(r"Pages wired down:\s+(\d+)")
...
class RazeWindow(QMainWindow):
    ...
    def _make_titlebar(self):
        ...
        for sym, slot in [("\u2014", self.showMinimized), ("\u25a1", self._toggle_max), ("\u00d7", self.close)]:
            ...
            b.setStyleSheet(
                ...
                f"font-family:'{_FONT_FAMILY}','{_FONT_FALLBACK}';}}"
    ...
    def _make_log_cell(self):
        ...
        prompt_lbl.setStyleSheet(
            f"color:{self.C['hi']}; font-size:13px; background:{self.C['bg1']}; border:none;"
